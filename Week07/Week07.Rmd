...
clSW %>% 
  ...
  geom_text(aes(label = perc), size = 3,
            ...
            inherit.aes = T) +
  coord_flip() +
  ggtitle("Which 'Star War' Movie have you seen")
...
clSW %>% 
  ...
  summarise(count = sum(charactor == 1, na.rm = T),
            perc = round(count/n() * 100, 2)) %>%
  drop_na() %>% 
  ...
  geom_text(aes(label = count), size = 3,
            ...
            inherit.aes = T) +
  coord_flip() +
  ggtitle("Which 'Star War' is most favorite")
  
```

### Plot "favorite charactor"
```{r favorite charactor}
clSW %>% 
